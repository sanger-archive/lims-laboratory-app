require('lims-laboratory-app/laboratory/spin_column/create_spin_column')
require('lims-laboratory-app/laboratory/spin_column/spin_column_persistor')
require('lims-laboratory-app/laboratory/spin_column/spin_column_sequel_persistor')
require('lims-laboratory-app/laboratory/spin_column')
require 'lims-laboratory-app/laboratory/spin_column/spin_column_resource'
