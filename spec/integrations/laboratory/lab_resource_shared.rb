...
shared_context "with saved sample" do
  ...
  let!(:sample) { Lims::LaboratoryApp::Laboratory::Sample.new(sample_name) }
  let!(:sample_uuid) {
...
shared_context "with labels" do
  ...
  let!(:labellable) {
    labellable = Lims::LaboratoryApp::Labels::Labellable.new(:name => resource_uuid,
                                                         :type => asset_type)
  }
  let!(:labellable_uuid) {
    '11111111-9999-3333-4444-888888888888'.tap do |labellable_uuid|
      store.with_session do |session|
        labellable[label_position_front] =
                Lims::LaboratoryApp::Labels::SangerBarcode.new({ :value => front_label_value })
        session << labellable
...
shared_context "with sample in location" do
  ...
  let(:new_aliquot_type) { 'sample' }
  let(:new_aliquot_quantity) { 10 }
  let!(:uuid) {
    "11111111-2222-3333-4444-555555555555".tap do |uuid|
      #save the flowcell
      sample_uuid
      store.with_session do |session|
        subject[sample_location] << Lims::LaboratoryApp::Laboratory::Aliquot.new(:sample => session[sample_uuid], 
                                                                        :type => new_aliquot_type,
                                                                        :quantity => new_aliquot_quantity)
        session << subject
